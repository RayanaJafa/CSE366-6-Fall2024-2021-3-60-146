...
class Environment:
    def __init__(self, num_classes, num_students, num_time_slots):
        ...
        self.student_availability = [np.random.choice([0, 1], size=num_time_slots, p=[0.2, 0.8]) for _ in range(num_students,24)]
        self.student_preferences = [np.random.randint(1, 6, size=num_time_slots) for _ in range(num_students,24)]
    ...
    def draw_schedule(self, screen, font, schedule):
        ...
        for student in range(self.num_students):
            ...
            availability_text = font.render(f"Student {student + 1} (Available: {sum(self.student_availability[student])})",True,(0, 0, 0),)
            screen.blit(availability_text, (10, margin_top + student * cell_size + 20))

            for slot in range(self.num_time_slots):
                rect = pygame.Rect(margin_left + slot * cell_size,margin_top + student * cell_size,cell_size,cell_size,)
                ...
                if assigned_class is not None:
                    ...
                    color = color = (0, 255 - priority * 40, 255)  # Gradient from light blue to dark blue
  

                    pygame.draw.rect(screen, color, rect)

                    # Display class info
                    priority_text = font.render(f"P{priority}", True, (255, 255, 255)if assigned_class==student else(0,0,0))
                    duration_text = font.render(f"{self.class_durations[assigned_class]}h", True, (255, 255, 255)if assigned_class==student else(0,0,0))
                    screen.blit(priority_text, (rect.x + 5, rect.y + 5))
                    screen.blit(duration_text, (rect.x + 5, rect.y + 25))
